...
export default function Post({ post, comments }) {
            ...
            <div ref={postRef} className="fauxmat-post-masthead">
                ...
                <div id="fauxmat-post-author">
                    {currentPost['_embedded']['author'][0] && (
                        <span>
                            <img
                                className="fauxmat-post-author-avatar"
                                alt="avatar"
                                src={currentPost['_embedded']['author'][0].avatar_urls[48]}
                            />
                ...
                <div id="fauxmat-post-comment-teaser">
                    ...
                    {post.modified !== post.date && (
                        <span>
                            last updated <Moment format="MMMM D, YYYY" date={post.modified} />
                        </span>
                    )}
                </div>
            </div>
    ...
    const getMastImage = (currentPost) => {
        return {
            ...
            backgroundImage:
                'url(' + currentPost['_embedded']['wp:featuredmedia'][0].source_url + ')'
        }
    }
    ...
    const getComment = (thread) => {
        return (
            <div key={`comment-${thread.id}`} className="fauxmat-comment-body-wrapper">
                ...
                <div
                    className="fauxmat-comment-body"
                    dangerouslySetInnerHTML={{ __html: thread.content.rendered }}
                />
            </div>
        )
    }
    ...
    const commentMessage =
        (!singleComment ? '' : comments.length) + ' Comment' + (singleComment ? 's' : '')
    return (
        <div id="fauxmat-post">
            {post && post.title && (
                <div id="fauxmat-post-inner">
                    ...
                    <div id="fauxmat-post-content-wrapper">
                        <div
                            id="fauxmat-post-content"
                            dangerouslySetInnerHTML={{ __html: post.content.rendered }}
                        />

                        {comments.length > 0 && (
                            <div ref={commentRef} id="fauxmat-post-comments-wrapper">
                                <h2>{comments.length && commentMessage}</h2>
                                <div id="fauxmat-post-comments-replies">
                                    {comments
                                        .filter((c) => c.parent === 0)
                                        .map((thread) => {
                                            const childThreads = comments.filter(
                        ...
                        <div
                            id="fauxmat-post-comment-teaser"
                            onClick={() => scrollToTop()}
                            onKeyDown={() => scrollToTop()}
                            role="button"
                            tabIndex="0">
